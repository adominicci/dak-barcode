...
- Decision: Standalone and combined-scan barcode counter payloads without a usable `LoadNumber` are rejected before mapping. Separate post-scan counter refresh failures are swallowed inside the optional counter refresh so required detail/union refreshes still settle before queued scans drain.
...
- Impacted areas: `src/lib/types/index.ts`, `src/lib/types/raw-dst.ts`, `src/lib/types/raw-dak.ts`, `src/lib/server/type-mappers.ts`, `src/lib/server/dst-queries.ts`, `src/lib/load-view.remote.ts`, `src/lib/server/dak-scan.spec.ts`, `src/routes/(app)/loading/+page.svelte`, `src/routes/(app)/loading/loading-page.svelte.spec.ts`, `docs/project-state.yaml`, `docs/current-context.md`, `/Users/andresdominicci/Projects/CustomerPortalAPI-PY/src/barcode_module/routes/barcode_update.py`, `/Users/andresdominicci/Projects/CustomerPortalAPI-PY/tests/barcode_module/test_process_loading_scan_v2.py`
