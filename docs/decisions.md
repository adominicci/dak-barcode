...
- Decision: Barcode counter payloads without a usable `LoadNumber` are rejected before mapping. Separate post-scan counter refresh failures are swallowed inside the optional counter refresh so required detail/union refreshes still settle before queued scans drain.
