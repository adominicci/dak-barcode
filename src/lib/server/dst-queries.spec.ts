...
describe("dst query helpers", () => {
  ...
  it("loads trailers through fetchDst and maps the canonical contract", async () => {
    fetchDst.mockResolvedValue(
      jsonResponse([
        {
          TrailerID: 9,
    ...
    const { getDstTrailers } = await import("./dst-queries");

    await expect(getDstTrailers()).resolves.toEqual([
      {
        id: 9,
        name: "TR-9",
      },
    ...
    expect(fetchDst).toHaveBeenCalledOnce();
    expect(getFetchCall()[0]).toBe("/api/barcode-get/get-trailers");
  });
  ...
  it("updates trailer and picked-by assignments with the legacy payloads", async () => {
    ...
    const { updateDstDropSheetPickedByLoader, updateDstDropSheetTrailer } =
    ...
    expect(fetchDst).toHaveBeenCalledTimes(2);

    const [trailerPath, trailerInit] = fetchDst.mock.calls[0] as [
      string,
      RequestInit | undefined,
    ];
    const trailerHeaders = new Headers(trailerInit?.headers);

    expect(trailerPath).toBe("/api/barcode-update/update-trailer-dropsheet");
    expect(trailerInit?.method).toBe("POST");
    expect(trailerHeaders.get("Content-Type")).toBe("application/json");
    expect(JSON.parse(String(trailerInit?.body))).toEqual({
      TrailerID: 9,
      dropsheetid: 42,
    });

    const [loaderPath, loaderInit] = fetchDst.mock.calls[1] as [
      string,
      RequestInit | undefined,
    ];
    const loaderHeaders = new Headers(loaderInit?.headers);

    expect(loaderPath).toBe("/api/dropsheet/update-picked-by");
    expect(loaderInit?.method).toBe("POST");
    expect(loaderHeaders.get("Content-Type")).toBe("application/json");
    expect(JSON.parse(String(loaderInit?.body))).toEqual({
      dropsheet_id: 42,
      picked_by: "Alex",
    });
  });
  ...
  it("loads dropsheet category availability through the legacy label-count endpoint", async () => {
    ...
    const { getDstDropSheetCategoryAvailability } =
      await import("./dst-queries");
  ...
  it("returns null for invalid drop-area lookups instead of leaking an empty backend record", async () => {
    ...
    expect(getFetchCall()[0]).toBe(
      "/api/barcode-get/get-droparea?dropareaid=999",
    );
  });
  ...
  it("normalizes category-list responses to the canonical operational departments", async () => {
    fetchDst.mockResolvedValue(
      jsonResponse(["Roll", { category: "Wrap" }, { Department: "Parts" }]),
    );

    const { getDstCategoryList } = await import("./dst-queries");

    await expect(getDstCategoryList()).resolves.toEqual([
      "Roll",
      "Wrap",
      "Parts",
    ]);
  ...
  it("loads staging day queries into the canonical staging list item contract", async () => {
    ...
    const { getDstStagingPartsForDay, getDstStagingPartsForDayRoll } =
      await import("./dst-queries");
    ...
    expect(url.pathname).toBe(
      "/api/barcode-get/get-staging-parts-for-day-roll",
    );
...
describe("dst query schemas", () => {
  it("validates scalar query inputs used by the remote wrappers", async () => {
    ...
    expect(
      v.safeParse(dropSheetDateSchema, "2026-03-23T12:00:00Z").success,
    ).toBe(false);
    ...
    expect(
      v.safeParse(dropSheetTrailerUpdateInputSchema, {
        dropSheetId: 42,
        trailerId: 7,
      }).success,
    ).toBe(true);
    ...
    expect(
      v.safeParse(dropSheetPickedByLoaderUpdateInputSchema, {
        dropSheetId: 42,
        loaderName: "Alex",
      }).success,
    ).toBe(true);
