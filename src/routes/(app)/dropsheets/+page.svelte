<script lang="ts">
	...
	function openDropsheet(dropSheetId: number, loadNumber: string, dropWeight: number) {
		const searchParams = new URLSearchParams({
			loadNumber,
			deliveryNumber: loadNumber,
			dropWeight: String(dropWeight)
		});
	...
	{:else}
		<div class="grid gap-3">
			{#each dropsheets as dropsheet (dropsheet.id)}
				<button
					type="button"
					onclick={() => openDropsheet(dropsheet.id, dropsheet.loadNumber, dropsheet.dropWeight)}
